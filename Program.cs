    ...
    namespace Actual_Causality
    {
        internal class Program
        {
            ...
            static Dictionary<string, int> calculateValues(Dictionary<string, int> known, List<string>unknown, List<thisVar> vars, string modelString, KeyValuePair<string, int> intervention)
            {
                ...
                while (unknown.Count > 0 && iter<10)           // we continue until there are no values unknown anymore, therefore we calculated all values
                {
                    ...
                    for(int i=0; i<unknown.Count; i++)         // for every variable in unknown
                    {
                        ...
                        int value = tryGetValue(unknown[i], known, vars, modelString);       // we try to calculate the value in this iter
                    ...
                    if (intervention.Key == null) { Console.WriteLine("intervention empty"); }
