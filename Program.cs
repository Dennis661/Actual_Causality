...
namespace Actual_Causality
{
    internal class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            List<string> copyEV = endoVarNames;
            if (copyEV.Count == 0) Console.WriteLine("emty before");
            else Console.WriteLine("non empty before");

            Dictionary <string, int> originalValues = calculateValues(exoValues, endoVarNames, vars, mainString, defaultIntervention);

            if (copyEV.Count == 0) Console.WriteLine("empty after");
            else foreach (string v in copyEV)
                {
                    Console.WriteLine(v + "   omg yes");
                }
